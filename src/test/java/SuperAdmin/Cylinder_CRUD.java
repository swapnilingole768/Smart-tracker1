...
public class Cylinder_CRUD {
	...
	public void createCylinderSizeTest() {
		...
		given() // Include JWT token in the request header
				...
				.body(requestBody).when().post(auth.API_KEY + "/cylinders/cylinder-sizes").then().statusCode(201) // Assuming
																													// successful
																													// creation
																													// returns
																													// 200
	...
	public void getCylinderById() {
		...
		given() // Include JWT token in the request header
				...
				.body(requestBody).when().get(auth.API_KEY + "/cylinders/cylinder-sizes/" + CylinderId).then()
				.statusCode(200) // Assuming successful creation returns 200 OK
				.log().all();
	}
